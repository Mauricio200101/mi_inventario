...
if st.session_state["menu_activo"] == "Inicio":
    ...
    if rol == "Administrador":
        ...
        with col_eq1:
            if st.button(
                "🖨️ Gestión de Equipos\n\nAlquileres, clientes y contadores",
                key="card_equipos"
            ):
...
# ----------------------------------------------------------------------------------
    if seccion == "Gestión de Equipos":
        st.subheader("🖨️ Gestión de Equipos")
        st.caption("Administra equipos por modalidad, clientes, contadores y rendimiento.")

        df_equipos = obtener_equipos()

        st.markdown("### 🗂️ Tipo de gestión")
        vista_equipos = st.radio(
            "Selecciona qué equipos quieres administrar:",
            ["🏢 Alquileres", "👤 Clientes", "🖨️ Todos"],
            horizontal=True,
            key="vista_gestion_equipos"
        )

        if not df_equipos.empty and "Modalidad" in df_equipos.columns:
            modalidad_norm = df_equipos["Modalidad"].astype(str).str.strip().str.lower()
            if vista_equipos == "🏢 Alquileres":
                df_equipos = df_equipos[modalidad_norm == "alquiler"].copy()
            elif vista_equipos == "👤 Clientes":
                df_equipos = df_equipos[modalidad_norm != "alquiler"].copy()
